...
public class AppRater {
    ...
    public MaterialDialog show() {
        ...
        if (launch_count >= mLaunchesBeforePrompt) { // wait at least x app launches
            if (System.currentTimeMillis() >= (firstLaunchTime + (mDaysBeforePrompt * DateUtils.DAY_IN_MILLIS))) { // wait at least x days
                try {
                    MaterialDialog dialog = showDialog(mContext, editor, firstLaunchTime);
                    return dialog;
                }
                catch (Exception e) {
                    return null;
    ...
    private MaterialDialog showDialog(final Context context, final SharedPreferences.Editor editor, final long firstLaunchTime) {
        final MaterialDialog rateDialog = new MaterialDialog.Builder(context)
                .title(mText_title)
                .content(mText_explanation)
                .positiveText(mText_buttonNow)
                .positiveColor(ContextCompat.getColor(context, R.color.accent))
                .neutralText(mText_buttonLater)
                .neutralColor(ContextCompat.getColor(context, R.color.black))
                .negativeText(mText_buttonNever)
